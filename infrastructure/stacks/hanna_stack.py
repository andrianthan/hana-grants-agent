...
class HannaStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # --- CfnParameters ---
        # WARNING: AllowedIps defaults to 0.0.0.0/0 for development use only.
        # Override at deploy time with your office/VPN CIDR for production:
        #   cdk deploy --parameters AllowedIps=<your-ip>/32
        allowed_ips_param = CfnParameter(
            self, "AllowedIps",
            type="String",
            default="0.0.0.0/0",
            description="CIDR range allowed to connect to RDS on port 5432. Default 0.0.0.0/0 is for development only -- restrict to your IP or VPN CIDR in production.",
        )
        ...
        # Secrets Manager rotation is deferred to Phase 2.
        # TODO Phase 2: Add rotation Lambda after scraper Lambda is deployed in same VPC.
        # Rotation requires the rotation Lambda and the secret to be in the same VPC
        # (or use Secrets Manager VPC endpoint). For now, rotate manually if needed:
        #   aws secretsmanager rotate-secret --secret-id <DbSecretArn>
        # Note: DB password is auto-generated by CDK; retrieve with:
        ...
        # Daily ingestion: runs at 1:00 PM UTC (Phase 2 enables this)
        ...
        # Weekly evaluation: runs Monday 3:00 PM UTC (Phase 3 enables this)
        ...
        # Email subscription -- requires manual confirmation from alert_email_param
        # after cdk deploy: check inbox for SNS confirmation email
        billing_topic.add_subscription(
            sns_subs.EmailSubscription(alert_email_param.value_as_string)
        )

        # Billing alert CloudWatch alarm must be created in us-east-1 (not us-west-2).
        # CDK cannot create cross-region alarms. Create manually after deploy:
        #
        #   TOPIC_ARN=$(aws cloudformation describe-stacks --stack-name HannaStack \
        #     --query "Stacks[0].Outputs[?OutputKey=='BillingAlertsSnsTopicArn'].OutputValue" \
        #     --output text)
        #
        #   aws cloudwatch put-metric-alarm --region us-east-1 \
        #     --alarm-name "HannaMonthlyBilling" \
