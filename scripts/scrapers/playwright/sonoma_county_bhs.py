"""Sonoma County Behavioral Health Division scraper.

Scrapes https://sonomacounty.ca.gov/bhd for county behavioral health
contracts and subgrants for community mental health services.
"""
    ...
    async def _scrape_page(self, page: Page) -> list[RawGrant]:
        ...
        for item in items:
            ...
            grants.append(RawGrant(
                title=title,
                funder="Sonoma County Behavioral Health",
                description=desc,
                deadline=deadline,
            ...
            if main:
                ...
                for link in links:
                    ...
                    if title and len(title) >= 10 and (
                        "rfp" in title.lower() or "contract" in title.lower()
                        or "grant" in title.lower() or "subgrant" in title.lower()
                    ):
                        grants.append(RawGrant(
                            title=title,
                            funder="Sonoma County Behavioral Health",
        ...
        logger.info(f"sonoma_county_bhs: found {len(grants)} opportunities")
        return grants
